...
void* multiplier_thread(void* arg) {
    ...
    while (multiplier_running) {
        ...
        // envia multiplicador para todos os clientes
        for (int i = 0; i < num_clients; i++) {
        ...
        // verifica se chegou ao ponto de explosão
        if (current_multiplier >= explosion_point_global) {
            ...
            msg_exp.player_id    = 0;
            msg_exp.player_profit= 0.0f;
            msg_exp.house_profit = 0.0f;
            // envia explode para todos os clientes
            for (int i = 0; i < num_clients; i++) {
        ...
        if (count == sizeof(aviator_msg)) {
            ...
            if (strcmp(received_msg.type, "bet") == 0) {
                if (can_bet) {
                    ...
                    response_msg.player_profit = 0.0f;
                    response_msg.house_profit  = 0.0f;
                } else {
                    // Rejeita a aposta
